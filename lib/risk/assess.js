...
(function () {
  ...
  Assess.execute = function (options, fn) {
    ...
    // GATHER LINE ITEMS
    var lineItems = [];
    options.lineItems.forEach(function (item) {
      var lineItemData = {
        _attributes: {
    ...
    options.externalRiskResults.forEach(function (external) {
      var riskData = {};

      if (external.score) {
        riskData.Score = {
          _text: external.score
        };
      }
    ...
    if (externalRisk.length) postBodyJson.Order.ExternalRiskResults.ExternalRiskResult = externalRiskScores;
    ...
    postBodyJson.ServerInfo.DSTActive = {
      _text: options.serverInfo.dstActive
    };
    ...
    if (options.deviceInfo) {
      ...
      if (options.deviceInfo.httpHeaders) {
        postBodyJson.DeviceInfo.HttpHeaders.HttpHeader = [];
